...
class STACDownloader:
    ...
    def download_bands(self, item, output_dir, bands=None, progress_callback=None):
        """
        Download spectral bands to output_dir.

        progress_callback(fraction: float, band_display: str, index: int, total: int)
            Called after each band completes.  fraction is 0.0–1.0.
        """
        output_dir = Path(output_dir)
        output_dir.mkdir(parents=True, exist_ok=True)

        if bands is None:
            bands = ['blue', 'green', 'red', 'rededge1', 'rededge3', 'nir', 'nir08', 'swir16']
        ...
        total = len(bands)
        downloaded = {}
        for i, asset_key in enumerate(bands):
            band_name  = band_mapping.get(asset_key, asset_key)
            label      = band_labels.get(asset_key, band_name)
